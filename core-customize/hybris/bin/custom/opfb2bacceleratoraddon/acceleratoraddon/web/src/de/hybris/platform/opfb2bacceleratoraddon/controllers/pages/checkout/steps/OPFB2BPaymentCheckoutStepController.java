...
public class OPFB2BPaymentCheckoutStepController extends AbstractCheckoutStepController {
    ...
    public String enterStep(final Model model, final RedirectAttributes redirectAttributes) throws CMSItemNotFoundException {
        ...
        final CartData cartData = opfCheckoutPaymentFacade.getOPFCheckoutCart();
        ...
        return Opfb2bacceleratoraddonControllerConstants.Views.Pages.MultiStepCheckout.ChooseOpfPaymentPage;
    }
    ...
    @ResponseBody
    @RequireHardLogIn
    public OPFPaymentSubmitCompleteResponseData submit(final HttpServletRequest request, final HttpServletResponse response,
            @PathVariable final String paymentSessionId, @RequestBody final OPFPaymentSubmitCompleteRequest paymentRequest) {
        try {
    ...
    public OPFPaymentSubmitResponseDTO submitPayment(final HttpServletRequest request, final HttpServletResponse response,
        ...
        if (opfPaymentSubmitRequestDTO == null || (!OPFAcceleratorCoreUtil.isQuickBuy(
                opfPaymentSubmitRequestDTO.getPaymentMethod()) && StringUtils.isEmpty(paymentSessionId))) {
            LOG.error("Invalid payment session ID or payment request data");
            throw new OPFRequestValidationException("Some required fields are missing or contain errors");
        }
        try {
            String ipAddress = request.getRemoteAddr();
            OPFPaymentSubmitResponseDTO opfPaymentSubmitResponseDTO = opfAcceleratorFacade.submitPayment(opfPaymentSubmitRequestDTO,
                    paymentSessionId, ipAddress);
            response.setStatus(HttpServletResponse.SC_CREATED);
