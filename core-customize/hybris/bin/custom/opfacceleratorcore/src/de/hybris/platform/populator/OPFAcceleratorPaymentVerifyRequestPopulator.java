...
public class OPFAcceleratorPaymentVerifyRequestPopulator implements Populator<OPFPaymentVerifyRequestDTO, OPFPaymentVerifyRequest> {
    ...
    private static final String OPF_ACTIVE_CONFIG_DIVISION_ID = "opf.active.config.division.id";
    @Resource(name = "configurationService")
    private ConfigurationService configurationService;
    private static final String PAYMENT_SESSION_ID_KEY = "opfPaymentSessionId";
    @Override
    public void populate(OPFPaymentVerifyRequestDTO source, OPFPaymentVerifyRequest target) throws ConversionException {
        ...
        target.setPaymentSessionId(String.valueOf(source.getResponseMap().stream()
                .filter(data -> StringUtils.equals(data.getKey(), PAYMENT_SESSION_ID_KEY))
                .findFirst()
                .map(OPFPaymentAttribute::getValue)
                .orElse(StringUtils.EMPTY)));
        target.setDivisionId(configurationService.getConfiguration().getString(OPF_ACTIVE_CONFIG_DIVISION_ID, StringUtils.EMPTY));
